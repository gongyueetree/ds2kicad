...
**核心原则（与 AltPart AI / PCB Quote AI / CKF 一致）：AI 只做语义提取，确定性规则引擎负责全部几何数学。**

- Gemini 仅返回：管脚表（照抄 Pin Configuration 表）、封装公称尺寸候选、图区页码 + 归一化包围盒。
- 符号布局、焊盘坐标、丝印/庭院/Fab 层、3D 几何全部由 `lib/kicadgen` 纯函数计算，AI 无权触碰。
- 图区包围盒仅是 AI 候选，用户在渲染页面上拖拽重新框选后才截图（截图由前端 pdf.js 在本地完成，像素级精确）。
- Gemini 响应经 3 次重试（指数退避）+ `repairJSON()`（剥围栏 / 闭合截断字符串与括号 / 补悬空冒号），`maxOutputTokens=16384` 防截断。
