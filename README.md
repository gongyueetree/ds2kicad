...
- **四类图标注**：内部功能框图 / 管脚排布图 / **封装图（新增）** / 应用参考电路（可多张，上限 3）。图集卡片上可直接下拉改类型。
- **可审核图集**：每张图支持「确认保留 / 丢弃 / 重新框选 / ＋－留白 / 点击放大」；只有**已确认**的图进入导出与 ezPLM。
- **裁剪留白**：程序化定位的 bbox 统一外扩（横 3%、纵 2%），AI 提示词也要求"宁大勿切"，解决管脚标注被裁掉；仍不理想时用卡片上的「＋留白」逐级扩大，或跳回 ④ 重新框选。
