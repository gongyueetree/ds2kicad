...
// item 9：生产环境必须显式配置，否则模块加载即失败（不允许静默用随机值导致令牌不可复现）
const IS_PROD = process.env.NODE_ENV === 'production' || process.env.VERCEL_ENV === 'production';
if (IS_PROD && !process.env.PDF_TOKEN_SECRET) {
  throw new Error('[DS2KiCad] 生产环境必须配置 PDF_TOKEN_SECRET，否则 PDF 取回令牌不可复现');
}
