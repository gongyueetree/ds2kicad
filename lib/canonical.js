...
import { enumerateAssetKeys } from './lifecycle.js';
 ...
 * 调用方拿到结果后做**原子提交**。
 */
export function runCanonicalPipeline({ job, patch, session, includeIds, markReviewed, finalRevision, finalState, figureBlobs = null }) {
  ...
  if (applied.changeLog.length) {
    ...
    // （若上游已 fork 为 draft(edited)，transition('edit') 仍返回 edited，幂等）
  ...
  try {
    bundle = generateBundle({
      ...
      assetKeys: enumerateAssetKeys(normalizedIr),
      items
    });
