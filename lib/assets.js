...
import { wrapLegacyLibrary } from './kicadgen/symbol.js';
 ...
 */
export function assembleAssets({ ir, bundle, job, reviewer, figureBlobs = null }) {
  const files = [];
  // DSK-001：同路径写入两份不同内容时，此前只会在 verifyConsistency 那里表现为
  // 一句语焉不详的「文件哈希不一致 X.lib」。这里就地拦截并说清是谁撞了名。
  const seenPaths = new Map();   // path → sha256
  const push = (path, content) => {
    const safe = safeZipPath(path, { fallback: 'asset' });
    const digest = sha(content);
    const prev = seenPaths.get(safe);
    if (prev !== undefined) {
      if (prev === digest) return safe;      // 完全相同，复用同一条目
      throw new Error(`资产路径冲突：${safe} 被写入两份不同内容（符号变体或封装重名）`);
    }
    seenPaths.set(safe, digest);
    files.push({ path: safe, content, sha256: digest, bytes: Buffer.byteLength(content, 'utf8') });
    return safe;
  };
  ...
  const symbolFiles = bundle.symbols.map((s) => ({
    name: s.name,
    packages: s.packages,
    packageIds: s.packageIds || [],       // item 13
    pinsetIds: s.pinsetIds || [],
    legacyPath: push(`${safeFileName(s.name)}.lib`, wrapLegacyLibrary(s.legacyLib, { name: s.name }))
  }));

  const itemFiles = bundle.items.map((it) => {
    // item 13：全程使用 packageId / pinsetId，禁止用名称回查资产
    const src = (ir.packages || []).find((p) => p.packageId === it.packageId);   // item 13：仅按 ID
    const entry = {
      packageId: it.packageId || src?.packageId || null,
      pinsetId: src?.pinsetId || null,
      packageName: it.pkgName, symbolName: it.symbolName, family: it.family,
      assetFlags: it.assetFlags, promotion: it.promotion
    };
  ...
  // item 8/9：图区 PNG 来自**对象存储**（IR 只存对象键）；内容由调用方注入 figureBlobs
  for (const f of ir.figures || []) {
    ...
    const b64 = figureBlobs?.[f.image.objectKey] || null;
    files.push({
      path: p, content: b64, encoding: 'base64',
      sha256: f.image.sha256, bytes: f.image.bytes,
      objectKey: f.image.objectKey, contentType: f.image.contentType || 'image/png'
    });
    const target = partBundle.figures.find((x) => x.figureId === f.figureId);
    if (target) {
      target.imagePath = p;
      target.imageSha256 = f.image.sha256;
      target.image = { objectKey: f.image.objectKey, sha256: f.image.sha256, bytes: f.image.bytes, width: f.image.width, height: f.image.height, contentType: f.image.contentType, documentSha256: f.image.documentSha256, page: f.image.page, bbox: f.image.bbox };
    }
  }
  ...
  // manifest.json 自身不列入 manifest.files（自指），但作为可下载文件返回；
  // verifyConsistency 按 assetFiles（不含 manifest.json）比对，口径一致。
  const manifestJson = JSON.stringify(manifest, null, 2);
  const manifestFile = { path: 'manifest.json', content: manifestJson, sha256: sha(manifestJson), bytes: Buffer.byteLength(manifestJson) };

  return { partBundle, manifest, files, manifestFile, allFiles: [...files, manifestFile], paths: { symLibPath, bundlePath } };
}
