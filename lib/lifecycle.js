...
const TRANSITIONS = {
  ...
  // item 2：approved 下允许**继续分批批准**（保持 approved），也允许 review 再确认
  [STATE.APPROVED]: { approve: STATE.APPROVED, review: STATE.REVIEWED, edit: STATE.EDITED, publish: STATE.PUBLISHED, revoke: STATE.REVOKED },
  // published 下允许继续发布其余已批准资产，或继续批准其余资产
  [STATE.PUBLISHED]: { publish: STATE.PUBLISHED, approve: STATE.PUBLISHED, edit: STATE.EDITED, revoke: STATE.REVOKED },
  [STATE.REVOKED]: {}
};
...
/** item 4：判断 Patch 是否触碰了已发布资产；返回受影响的已发布资产键 */
export function publishedAssetsTouched(ir, changeLog = []) {
  const published = ir?.lifecycle?.published || {};
  if (!Object.keys(published).length || !changeLog.length) return [];
  const probe = invalidateAffectedApprovals(ir, changeLog);
  return probe.invalidated
    .filter((k) => k.startsWith('published:'))
    .map((k) => k.slice('published:'.length));
}

/** item 4：为已发布作业创建新的 draft revision（保留已发布版本记录，state 回到 edited） */
export function forkDraft(ir, { actor, reason, at = new Date().toISOString() }) {
  const next = structuredClone(ir);
  const prevPublished = next.lifecycle?.published || {};
  next.draftOf = {
    fromState: next.lifecycle?.state || 'published',
    publishedSnapshot: prevPublished,
    forkedBy: actor, forkedAt: at, reason: reason || ''
  };
  next.lifecycle = {
    ...(next.lifecycle || {}),
    state: STATE.EDITED,
    approvals: {},           // 新 draft 不继承批准
    published: {},           // 已发布记录移入 draftOf.publishedSnapshot（AssetVersion 仍不可变留存）
    reviewedBy: null,
    history: [...(next.lifecycle?.history || []), { action: 'fork_draft', from: next.lifecycle?.state, to: STATE.EDITED, actor, reason, at }]
  };
  return next;
}
