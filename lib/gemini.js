...
export function buildPrompt(need, hints = {}) {
  ...
  if (need.packages) {
    schema.push(`"packages": [ { "name": "e.g. WQFN-24", "tiCode": "vendor code or empty", "type": "WQFN / TSSOP / SOIC / PDIP / SOT-23 ...", "pinsetId": "default", "pinCount": 24, "pitch": 0.5, "bodyLength": 4.0, "bodyWidth": 4.0, "height": 0.8, "leadSpan": 4.0, "leadLength": 0.4, "leadWidth": 0.25, "epLength": 2.6, "epWidth": 2.6 } ],
  "recommendedPackageIndex": 0`);
    rules.push(`List EVERY package variant offered in the datasheet as a separate entry in "packages" (e.g. SOIC, PDIP, TSSOP, VSSOP each get their own entry with their own dimensions). Package dimensions come from the Mechanical/Package outline drawing, in millimeters, NOMINAL (typical) values. "bodyLength" is along the pin rows; "leadSpan" is lead-tip-to-lead-tip for gull-wing (equals body size for QFN). If a value is unavailable, use the JEDEC-standard nominal for that package type.`);
 ...
 */
export async function extractWithGemini({ pdfBase64, apiKey, model, sourceUrl, need, hints }) {
