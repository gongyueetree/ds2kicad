...
export function buildPrompt(need, hints = {}) {
  ...
  if (need.figures) {
    schema.push(`"figures": [ { "kind": "block_diagram" | "pin_configuration" | "package_outline" | "application", "title": <string>, "page": <int>, "bbox": [<x0>, <y0>, <x1>, <y1>] } ]`);
    rules.push(`"figures": ONLY include figures whose printed caption/title text literally contains these keywords — (a) "Block Diagram" (functional/internal/simplified variants OK): exactly 1; (b) "(Top View)" or "Pin Configuration" package drawings (kind "pin_configuration"): one per package variant; (c) "Package Outline" / "Mechanical Drawing" / "Land Pattern Example" / 封装外形 / 机械图 (kind "package_outline"): at most one per package variant;
(d) "Typical Application" / "Application Circuit" / "Simplified Schematic": at most 3, the most representative ones. Chinese-language datasheets use equivalent caption keywords which count as matches: 框图/功能框图/内部框图 = Block Diagram; 典型应用/应用电路/应用示例/简化原理图 = Typical Application; 引脚配置/引脚排列/顶视图 = Pin Configuration (Top View); caption prefix may be "图 N-M." instead of "Figure N-M.". STRICTLY EXCLUDE performance curves, waveforms, "Application Curves"/曲线/波形/特性, frequency-response plots, X-vs-Y graphs, tables, and any figure that merely looks like an image — keyword match on the caption is the ONLY criterion. Copy the caption verbatim into "title". "page" is 1-based and MUST be exact — it is verified against the rendered page. "title" MUST be the printed caption copied VERBATIM, character for character, including any "Figure N-M." / "图 N-M." prefix and the original language; the client anchors the crop rectangle on this exact string in the PDF text layer, so a paraphrased or translated title breaks figure extraction. "bbox" is [x0,y0,x1,y1] normalized with origin TOP-LEFT, and is only a COARSE HINT used as a fallback seed — the client recomputes the true rectangle deterministically from page geometry, so do not agonise over it; getting "page" and a verbatim "title" right matters far more than bbox precision.`);
  }
  const hintLines = [];
...
export async function extractWithGemini({ pdfBase64, apiKey, model, sourceUrl, need, hints, deadlineMs }) {
  // item 10：可注入 Stub —— 用于在不调用真实 Gemini 的前提下走通 live / degraded 服务端分支。
  // GEMINI_STUB=<json>   → 直接返回该 JSON（模拟成功的 live 提取）
  // GEMINI_STUB=throw:<msg> → 抛错（模拟 AI 不可用，触发 degraded 分支）
  const stub = process.env.GEMINI_STUB;
  if (stub) {
    if (stub.startsWith('throw:')) throw new Error(stub.slice(6) || 'stubbed gemini failure');
    try { return JSON.parse(stub); } catch (e) { throw new Error(`GEMINI_STUB 不是合法 JSON: ${e.message}`); }
  }
