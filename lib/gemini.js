...
export function buildPrompt(need, hints = {}) {
  ...
  if (need.packages) {
    schema.push(`"packages": [ { "name": "e.g. WQFN-24", "tiCode": "vendor code or empty", "type": "WQFN / TSSOP / SOIC / PDIP / SOT-23 ...", "pinCount": 24, "pitch": 0.5, "bodyLength": 4.0, "bodyWidth": 4.0, "height": 0.8, "leadSpan": 4.0, "leadLength": 0.4, "leadWidth": 0.25, "epLength": 2.6, "epWidth": 2.6 } ],
  "recommendedPackageIndex": 0`);
    rules.push(`Package dimensions come from the Mechanical/Package outline drawing, in millimeters, NOMINAL (typical) values. "bodyLength" is along the pin rows; "leadSpan" is lead-tip-to-lead-tip for gull-wing (equals body size for QFN). If a value is unavailable, use the JEDEC-standard nominal for that package type.`);
  }
  if (need.pins) {
    schema.push(`"pins": [ { "number": "1", "name": "NO1", "type": "passive", "description": "..." } ]`);
    rules.push(`"pins" MUST be copied verbatim from the Pin Configuration and Functions table of the recommended package. Every physical pin (including exposed pad, e.g. number "25") appears exactly once. Never invent pin names. "type" is one of: input, output, bidirectional, power_in, power_out, passive, tri_state, open_collector, no_connect, unspecified. Analog switch channel pins are "passive"; logic control pins are "input"; VCC/VDD/GND are "power_in"; exposed pad is "passive".`);
  }
  if (need.figures) {
    schema.push(`"figures": [ { "kind": "block_diagram" | "application", "title": "...", "page": 1, "bbox": [0.15, 0.30, 0.85, 0.75] } ]`);
    rules.push(`"figures": locate (a) the functional/internal block diagram and (b) 1-3 typical application / simplified schematic figures. "page" is 1-based. "bbox" is [x0,y0,x1,y1] normalized, origin TOP-LEFT, generously enclosing the figure INCLUDING caption (+3% margin).`);
  }
  const hintLines = [];
