...
/** 管脚表解析：识别 Pin Functions/Pin Configuration 表，行按 (NAME NO TYPE DESC) 或 (NO NAME TYPE DESC) 两种列序尝试 */
export function parsePinTable(pages) {
  const rows = new Map(); // number → {name,type,description}
  let headerSeen = false;
  const rowReA = new RegExp(`^(${NAME_RE})\\s+(\\d{1,3})\\s+(I/O|IO|I|O|P|PWR|G|GND|NC|[-—–])\\s*(.*)$`);
  const rowReB = new RegExp(`^(\\d{1,3})\\s+(${NAME_RE})\\s+(I/O|IO|I|O|P|PWR|G|GND|NC|[-—–])\\s*(.*)$`);
  ...
  for (const pg of pages) {
    for (let i = 0; i < pg.lines.length; i++) {
      ...
      if (!headerSeen) {
        if (/^(\d+(\.\d+)*\s+)?Pin (Configuration and )?Functions?\b/i.test(t) ||
            (/\bPIN\b/.test(t) && /\b(DESCRIPTION|FUNCTION)\b/i.test(t))) headerSeen = true;
        continue;
      }
      if (stop.test(t)) { headerSeen = false; continue; }
      let mA = rowReA.exec(t), mB = rowReB.exec(t);
      // NAME 不能是纯数字（避免 A 模式吃掉 B 模式的行）
      if (mA && /^\d+$/.test(mA[1])) mA = null;
      const m = mA || mB;
      if (m) {
        const [name, num] = mA ? [m[1], m[2]] : [m[2], m[1]];
        const ty = (m[3] || '').toUpperCase();
        if (!rows.has(num)) {
          rows.set(num, {
            number: num,
            name,
            type: /^(GND|VSS)/i.test(name) || /^(VCC|VDD|V\+)/i.test(name)
              ? 'power_in'
              : (TYPE_MAP[ty] || 'unspecified'),
            description: (m[4] || '').trim().slice(0, 160)
          });
        }
      } else if (epRe.test(t) && rows.size >= 4) {
        const epNum = String([...rows.keys()].reduce((a, b) => Math.max(a, +b), 0) + 1);
        if (!rows.has(epNum)) rows.set(epNum, { number: epNum, name: 'EP', type: 'passive', description: 'Exposed thermal pad' });
      }
    }
  }

  const pins = [...rows.values()].sort((a, b) => +a.number - +b.number);
  // 置信度：行数≥4、编号从 1 起连续覆盖 ≥80%、无重名冲突迹象
  const max = pins.length ? +pins[pins.length - 1].number : 0;
  const coverage = max ? pins.length / max : 0;
  const confidence = pins.length >= 4 && max >= 6 && coverage >= 0.8 && +pins[0].number === 1 ? 'high' : 'low';
  return { pins, confidence, coverage: +coverage.toFixed(2) };
}
...
export function findFigures(pages) {
  ...
  for (const pg of pages) {
    ...
    for (let i = 0; i < sorted.length; i++) {
      const l = sorted[i];
      const cap = /^Figure\s+(\d+)[-.:]?\s+(.*)$/i.exec(l.text);
      const matchKind = (s) => FIG_KINDS.find((k) => k.re.test(s))?.kind;
      if (cap) {
        const kind = matchKind(cap[2]);
        if (!kind) continue;
        ...
        const kind = l.text.length < 60 && !/\b(see|shown|refer)\b/i.test(l.text) ? matchKind(l.text) : null;
        if (!kind) continue;
  ...
  // 排序去冗：block_diagram 优先且仅 1 个，application 最多 3 个
