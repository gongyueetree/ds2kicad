...
const FIG_KINDS = [
  { kind: 'block_diagram', re: /(functional|internal|simplified)?\s*block\s+diagram|功能框图|内部框图|方框图|结构框图/i },
  { kind: 'application', re: /(typical\s+application|application\s+(circuit|schematic|diagram)|simplified\s+(schematic|application))|典型应用|应用电路|应用示例|简化原理图/i },
  { kind: 'pin_configuration', re: /top\s+view|pin\s+(configuration|connections?|assignment)\s*(diagram)?\b(?!\s+and\s+functions)|引脚配置|引脚排列|管脚排列|顶视图|俯视图/i },
  { kind: 'package_outline', re: /package\s+(outline|drawing|dimensions)|mechanical\s+drawing|land\s+pattern\s+example|封装外形|封装图|机械图|推荐焊盘/i }
];
...
export function findFigures(pages) {
  ...
  for (const pg of pages) {
    ...
    for (let i = 0; i < sorted.length; i++) {
      const l = sorted[i];
      const cap = /^(?:Figure|图)\s*(\d+(?:[-.]\d+)*)\s*[.:、]?\s*(.*)$/i.exec(l.text);
      const matchKind = (s) => FIG_KINDS.find((k) => k.re.test(s))?.kind;
      if (cap) {
        let kind = matchKind(cap[2]);
        // 曲线/波形类说明行（如 "Application Curves"、"增益曲线"）不是电路图
        if (kind && (/\b(curve|curves|waveform|response|characteristic|performance|graph|plot|vs\.?|versus)\b/i.test(cap[2]) || /曲线|波形|特性|性能/.test(cap[2]))) kind = null;
        if (!kind) continue;
        ...
        // 独立标题（无 Figure 编号）误报率高：英文只认 simplified schematic / block diagram；
        // 中文标题（典型应用/功能框图等短标题）常直接作图题，放行；管脚表章节名不算图
        if (kind === 'pin_configuration') kind = null;
        if (kind === 'application' && !/simplified\s+schematic|典型应用|应用电路|应用示例/i.test(l.text)) kind = null;
        if (!kind) continue;
        // 中文说明常置于图的下方：标题在页面下半部 → 图在上方（按说明行处理）
        if (l.y < pg.height * 0.55) {
          let topY = Math.min(H * 0.94, l.y + H * 0.45);
          for (const t of sorted) {
            if (t.y <= l.y || t.y - l.y > H * 0.5) continue;
            if (t.text.length > 60 || /^\d+(\.\d+)*\s+[A-Z]/.test(t.text)) topY = Math.min(topY, t.y - t.h * 1.2);
          }
          pushFig(out, kind, l.text, pg.page, [0.06, 1 - topY / H, 0.94, Math.min(0.98, 1 - (l.y - l.h * 1.6) / H)]);
          continue;
        }
  ...
  // v0.8.9：此前漏掉 package_outline，导致只要需要封装图就整体回退到 Gemini 图区路径，
  // 而 Gemini 的 bbox 不可信 —— 解析器能覆盖的类型必须由解析器覆盖。
  const pkgout = out.filter((f) => f.kind === 'package_outline').slice(0, 2);
  const apps = out.filter((f) => f.kind === 'application').slice(0, 3);
  return [...blocks, ...pincfg, ...pkgout, ...apps];
}

/** v0.8.8：图区外扩留白（横向 3%、纵向 2%），避免管脚标注/引出线被裁掉 */
const PAD_X = 0.03, PAD_Y = 0.02;

function pushFig(arr, kind, title, page, bbox) {
  const pad = [bbox[0] - PAD_X, bbox[1] - PAD_Y, bbox[2] + PAD_X, bbox[3] + PAD_Y];
  const [x0, y0, x1, y1] = pad.map((v) => Math.min(0.995, Math.max(0.005, +v.toFixed(4))));
