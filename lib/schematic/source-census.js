  ...
  for (const page of pages || []) {
    ...
    for (const r of refs) {
      ...
      for (const p of parts) {
        ...
        if (dy < 0) score -= 7; // value/MPN is commonly drawn below the reference.
...
export function buildSourceCensus(textPages) {
  ...
  // Hard-gate labels are repeated protocol/power labels; single isolated pin names are too noisy.
