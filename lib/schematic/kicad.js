...
// Emits modern .kicad_sch plus legacy .sch/cache.lib fallback.
...
function pagePoint(pos = {}) {
  // A4 landscape-ish working area, kept away from title block/borders.
  return {
...
function pinLayout(pins = []) {
  ...
  const cols = Math.max(sides.top.length, sides.bottom.length, 2);
...
function symbolDefinition(libId, pins) {
  ...
  const out = [`    (symbol "${esc(libId)}" (pin_names (offset 0.508)) (in_bom yes) (on_board yes)`];
  out.push(`      (rectangle (start ${f(-g.halfW)} ${f(-g.halfH)}) (end ${f(g.halfW)} ${f(g.halfH)}) (stroke (width 0.254) (type default)) (fill (type background)))`);
  for (const it of g.items) {
    const p = it.pin;
    const type = TYPE.has(p.type) ? p.type : 'unspecified';
    out.push(`      (pin ${type} line (at ${f(it.x)} ${f(it.y)} ${PIN_ANGLE[it.side]}) (length ${f(g.pinLen)}) (name "${esc(p.name || p.number)}") (number "${esc(p.number)}"))`);
  }
  out.push('    )');
...
function compileDefinitions(ir) {
  ...
  for (const c of ir.components) {
    // Official hints are preserved as lib IDs; unknown ICs remain unique DS2KiCad IDs.
    const libId = c.libraryId || `DS2KiCad:${c.ref}`;
    if (!defs.has(libId)) defs.set(libId, symbolDefinition(libId, c.pins));
    compInfo.set(c.ref, { libId, geometry: defs.get(libId).geometry, at: pagePoint(c.position) });
  }
...
function modernProperties(c, at) {
  ...
  return [
    `    (property "Reference" "${esc(c.ref)}" (at ${f(at.x)} ${f(refY)} 0))`,
    `    (property "Value" "${esc(c.value || c.mpn || c.ref)}" (at ${f(at.x)} ${f(valY)} 0))`,
    `    (property "Footprint" "${esc(c.footprint || '')}" (at ${f(at.x)} ${f(at.y)} 0))`,
    `    (property "Datasheet" "" (at ${f(at.x)} ${f(at.y)} 0))`
  ];
}
...
export function generateModernSchematic(ir) {
  ...
  for (const c of ir.components) {
    ...
    lines.push(`  (symbol (lib_id "${esc(info.libId)}") (at ${f(info.at.x)} ${f(info.at.y)} 0) (unit 1) (in_bom yes) (on_board yes) (uuid ${randomUUID()})`);
    lines.push(...modernProperties(c, info.at));
    lines.push('  )');
  }

  // Same-name labels form the electrical graph on a sheet. This is deliberately more robust
  // than asking the model to synthesize long graphical wires around regenerated symbols.
  for (const net of ir.nets) {
    for (const ep of net.endpoints) {
      ...
      if (!p) continue;
      lines.push(`  (label "${esc(net.name)}" (at ${f(p.x)} ${f(p.y)} 0) (uuid ${randomUUID()}))`);
    }
  }
