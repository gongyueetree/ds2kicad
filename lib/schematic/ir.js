// Canonical Connectivity IR for PDF/image schematic extraction.
// Connectivity is the source of truth. KiCad schematics are only one renderer/export target.
import { randomUUID } from 'node:crypto';
import { analyzeConnectivity } from '../connectivity/analysis.js';

export const SCHEMATIC_IR_VERSION = 'connectivity-intelligence.ir.v1';
export const CONNECTIVITY_IR_VERSION = SCHEMATIC_IR_VERSION;
...
export function sanitizeSchematicIR(raw, meta = {}) {
  if (!raw || typeof raw !== 'object') throw new Error('connectivity extraction returned no object');
  const warnings = [];
  const components = [];
  const refs = new Set();
  ...
  for (let i = 0; i < inputComponents.length; i++) {
    ...
    const libraryId = clean(c.libraryId || c.library_id || inferred || `Connectivity:${ref}`, 120);
    const bbox = Array.isArray(c.bbox) && c.bbox.length === 4
      ...
      : null;
    const x = clamp01(c.position?.x ?? c.x ?? (bbox ? (bbox[0] + bbox[2]) / 2 : 0.5));
    const y = clamp01(c.position?.y ?? c.y ?? (bbox ? (bbox[1] + bbox[3]) / 2 : 0.5));
    components.push({
  ...
  if (!components.length) warnings.push('No components were recognized from the source.');
  if (components.length && !nets.length) warnings.push('Components were recognized, but no multi-endpoint nets were reconstructed.');

  const base = {
    schemaVersion: CONNECTIVITY_IR_VERSION,
    kind: 'connectivity-ir',
    title: clean(raw.title || meta.fileName || 'Connectivity Design', 160),
    source: {
  ...
  return { ...base, ...analyzeConnectivity(base) };
}
...
export function summarizeSchematicIR(ir) {
  ...
  return {
    ...
    interfaces: ir.interfaces?.length || 0,
    powerRails: ir.powerRails?.length || 0,
    confidence: ir.confidence,
    warnings: ir.warnings.length,
    issues: ir.issues?.length || 0,
    errors: ir.health?.error || 0,
    healthScore: ir.health?.score ?? 0
  };
}
