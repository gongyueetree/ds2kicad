...
// 输出按页的“行”结构（文本 + PDF 坐标），供 lib/heuristics.js 做确定性解析。
// 扫描版 PDF（无文本层）返回空行数组，上层自动回退全量 Gemini 路径。
import { getDocument } from 'pdfjs-dist/legacy/build/pdf.mjs';

/**
 * @returns {Promise<{pageCount:number, pages:Array<{page:number,width:number,height:number,lines:Array<{text:string,x:number,y:number,x1:number,h:number}>}>}>}
