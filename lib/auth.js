...
export function authenticate(req) {
  ...
  if (!key) {
    ...
    return { ok: true, session: { sub: 'dev-anonymous', name: '开发匿名用户', tenantId: 'dev', roles: ['viewer', 'editor', 'reviewer'], authenticated: false } };
  }
...
export function authorizeJobAccess(session, job, { requireRole } = {}) {
  if (!session?.authenticated) return { ok: false, status: 401, error: '需要已认证的 ezPLM 会话' };
