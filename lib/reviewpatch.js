...
function applyReviewPatchInner(ir, patch, { reviewer, at = new Date().toISOString(), markReviewed = true } = {}) {
  ...
  const KNOWN_TOP = new Set(['schemaVersion', 'part', 'pinsets', 'packages', 'figures', 'includePackageIds', 'expectedRevision', 'approvals']);
    ...
    if (!Array.isArray(patch.packages)) errors.push({ path: 'packages', error: 'packages 必须是数组' });
      ...
      for (const [k, entry] of Object.entries(item)) {
        ...
        target.fieldProvenance = {
          ...(target.fieldProvenance || {}),
          [k]: { source: 'reviewer', rawValue: target.fieldProvenance?.[k]?.rawValue ?? null, normalizedValue: n, reviewer: `${reviewer.name} <${reviewer.sub}>`, reason: reason || 'review_patch', at }
        };
