...
import { lintPinNames } from './pinlint.js';
...
export function sanitizePinsDetailed(pins) {
  ...
  // DSK-005 / DSK-006：管脚名规则校验。只自动修正无歧义的 OCR 字母/数字混淆，
  // 重名、脚注上标、曝光焊盘语义一律只标记不改写，交人工复核。
  const lint = lintPinNames(out);
  for (const it of lint.issues) log.push(it);
  return { pins: out, rawPins, transformationLog: log, reviewRequired: log.length > 0, pinIssues: lint.issues };
}
...
/* ── DSK-010：DIP 孔距（rowSpan）解析 ─────────────────────────────────
 * 缺陷：rowSpan 此前无条件默认 7.62（300 mil），且 DIP 从不使用已提取的 leadSpan，
 * 也不读封装名里的 "400 mil"。结果 CNY17F 的 DIP-6 (400 mil) 明明提取到
 * leadSpan=10.16，却生成了 7.62 的孔距 —— 名称写着 400 mil，几何却是 300 mil，
 * 通孔器件直接装不上板。3D 模型同样取 pkg.rowSpan，于是一并错。
 * 可信度排序：手册 land pattern > 显式 rowSpan > 封装名标称 > leadSpan > 默认值。
 */
const MIL = 0.0254;
export const DIP_STD_ROWSPAN = [7.62, 10.16, 15.24, 22.86];   // 300 / 400 / 600 / 900 mil

/** 从封装名/型号里解析标称孔距，如 "DIP-6 (400 mil)" → 10.16；"..._W7.62mm_..." → 7.62 */
export function parseRowSpanFromName(name) {
  const s = String(name || '');
  const mil = /(\d{3,4})[\s_-]*mil/i.exec(s);   // 兼容 '400 mil' / '400_mil' / '400-mil'
  if (mil) {
    const v = Number(mil[1]) * MIL;
    return Number.isFinite(v) && v >= 2.54 && v <= 30 ? +v.toFixed(3) : null;
  }
  const mm = /\bW(\d+(?:\.\d+)?)mm/i.exec(s);
  if (mm) {
    const v = Number(mm[1]);
    return Number.isFinite(v) && v >= 2.54 && v <= 30 ? v : null;
  }
  return null;
}

/** 贴近标准 DIP 孔距时吸附过去（手册常写 10.16/10.2/0.4in，避免生成非标值） */
export function snapDipRowSpan(v, tol = 0.35) {
  if (!Number.isFinite(v)) return v;
  for (const std of DIP_STD_ROWSPAN) if (Math.abs(v - std) <= tol) return std;
  return +v.toFixed(3);
}
...
export function sanitizePackage(pkg) {
  ...
  // ── DSK-010：DIP 孔距按可信度重解析，并与封装名做矛盾检查 ──
  if (family === 'dip') {
    const named = parseRowSpanFromName(out.name) ?? parseRowSpanFromName(out.type);
    const fromLp = Number.isFinite(lp?.rowSpan) ? lp.rowSpan : null;
    const explicit = prov.rowSpan?.source === 'datasheet' ? out.rowSpan : null;
    const fromLead = prov.leadSpan?.source === 'datasheet' ? out.leadSpan : null;

    let picked = null, src = null;
    if (fromLp !== null) { picked = fromLp; src = 'datasheet_land_pattern'; }
    else if (explicit !== null) { picked = explicit; src = 'datasheet'; }
    else if (named !== null) { picked = named; src = 'package_name'; }
    else if (fromLead !== null) { picked = fromLead; src = 'derived_from_leadSpan'; }

    // 封装名是手册自己的标称，与几何矛盾时以名称为准并留痕（"400 mil 却 7.62mm" 属物理矛盾）
    if (picked !== null && named !== null && Math.abs(picked - named) > 0.3) {
      record('rowSpan', {
        source: 'corrected', rawValue: picked, normalizedValue: named,
        reason: `row_span_contradicts_package_name(name=${named}mm, ${src}=${picked}mm)`
      });
      picked = named; src = 'package_name';
    }

    if (picked !== null) {
      const snapped = snapDipRowSpan(picked);
      if (prov.rowSpan?.source !== 'corrected') {
        record('rowSpan', { source: src === 'datasheet' ? 'datasheet' : 'derived', rawValue: picked, normalizedValue: snapped, reason: `resolved_from:${src}` });
      } else {
        prov.rowSpan.normalizedValue = snapped;
      }
      out.rowSpan = snapped;
      // land pattern 与 pkg.rowSpan 必须同源，否则封装名/3D/焊盘三者会各说各话
      if (lp && Number.isFinite(lp.rowSpan)) lp.rowSpan = snapped;
    }
  }
