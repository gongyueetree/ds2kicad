...
import { generateLgaFootprint, generateLgaWrl, isLgaPackage, promoteLgaPackage } from './lga.js';
...
// LGA is supported locally by the dedicated perimeter-LGA generator. Other families continue
// to use validate.js as the central allowlist.
import { UNSUPPORTED_FAMILY_NOTES, SUPPORTED_FAMILIES } from '../validate.js';
const GENERATOR_FAMILIES = new Set([...SUPPORTED_FAMILIES, 'lga']);
const unsupportedNoteFor = (pkg) =>
  (pkg.familySupported === false || !GENERATOR_FAMILIES.has(pkg.family))
...
export function generateBundle(input) {
  ...
  for (const it of rawItems) {
    ...
    const rawPkg = it?.pkg || {};
    let pkg = sanitizePackage(rawPkg);
    // validate.js historically rejected LGA because no deterministic generator existed. Promote only
    // explicitly-labelled LGA packages; unknown packages remain fail-closed.
    if (isLgaPackage(rawPkg)) pkg = promoteLgaPackage(pkg, rawPkg);
  ...
  // 2) 每封装生成封装 + 3D（关键几何缺失 → blocked）
  const outItems = items.map(({ pkg, pins, group }) => {
    const itemWarnings = [];
    const missing = pkg.missingFields || [];
    const unsupportedNote = unsupportedNoteFor(pkg);
    // LGA needs all perimeter geometry fields; legacy families keep the prior conservative rule.
    let blocked = pkg.family === 'lga'
      ? missing.length > 0
      : missing.includes('pitch') && (missing.includes('bodyLength') || missing.includes('bodyWidth'));
    if (blocked) {
      itemWarnings.push(`blocked_missing_geometry：数据手册未提供/未确认 ${missing.join('/') || '必要几何参数'}，已阻断封装/3D 生成；请在 ③ 手工补齐后重新生成`);
    }
    let single = null;
    if (!blocked && !unsupportedNote) {
      single = safeSingle({ part, pkg, pins }, itemWarnings);
      if (!single) blocked = true; // safeSingle 内部拒绝（如四边封装非 4 的倍数）
    }
    ...
    // 无手册 land pattern → 焊盘为规则推导；WRL 恒为参数化近似，不得冒充厂商 STEP
  ...
  return {
    ...
    items: outItems,
    mock: !!input.mock,
    ...evaluateBundle({ items: outItems, mock: !!input.mock, confirmedFigureCount: input.confirmedFigureCount, assetKeys: input.assetKeys || [] }),
    warnings
  };
}

/** 单封装生成（内部）：不支持家族跳过；四边封装必须可确定性还原 */
function safeSingle({ part, pkg, pins }, warnings) {
  ...
  const p = { ...pkg };
  if ((p.family === 'qfn' || p.family === 'lga') && p.pinCount % 4 !== 0) {
    warnings.push(`blocked_missing_geometry：${p.family.toUpperCase()} 引脚数 ${p.pinCount} 非 4 的倍数，无法确定性还原四边排布，已阻断封装/3D 生成`);
    return null;
  }
  const mpn = String(part.mpn || 'PART').trim() || 'PART';
  let g, transformations, geoWarn;
  // Dedicated LGA geometry is already validated by its generator; do not run SOIC/QFN heuristics on it.
  if (p.family === 'lga') {
    g = p; transformations = []; geoWarn = [];
  } else if (p.geometryNormalized) {
  ...
  const pinNumbers = pins.map((x) => String(x.number));
  const kicadMod = g.family === 'lga'
    ? generateLgaFootprint({ fpName, pkg: g, pinNumbers })
    : generateFootprint({ mpn, pkg: g, footprintNameOverride: fpName });
  const wrl = g.family === 'lga' ? generateLgaWrl({ pkg: g }) : generateWrl({ pkg: g });
