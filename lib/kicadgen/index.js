...
export function generateBundle(input) {
  ...
  const outItems = items.map(({ pkg, pins, group }) => {
    ...
    const unsupportedNote = UNSUPPORTED[pkg.family];
    ...
    const fpName = unsupportedNote || blocked ? null : footprintName(mpn, pkg);
  ...
  for (const g of groups.values()) {
    ...
    symbols.push({ name: g.symbolName, legacyLib: generateLegacyLib({ mpn: g.symbolName, pins: g.pins }), packages: g.packages, pins: g.pins });
  }
  ...
  return {
    ...
    items: outItems,
    mock: !!input.mock,
    ...evaluateBundle({ items: outItems, mock: !!input.mock }), // 唯一晋升闸门
    warnings
  };
}
...
function safeSingle({ part, pkg, pins }, warnings) {
  ...
  const mpn = String(part.mpn || 'PART').trim() || 'PART';
  const g = normalizeGeometry(p, warnings);
  return { kicadMod: generateFootprint({ mpn, pkg: g }), wrl: generateWrl({ pkg: g }) };
}
