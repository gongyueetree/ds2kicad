...
export function generateBundle(input) {
  ...
  const items = [];
  // DSK-001：symbolVariantName 由 tiCode/type/name 去符号大写而来，多 pinset 器件
  // （如 AP2112 的 4 个 pinset）极易撞名。重名 → assets 里两份不同内容写到同一个
  // `<name>.lib` 路径 → manifest 哈希对不上 → 生成阶段 422「文件哈希不一致」。
  const usedSymbolNames = new Set();
  const uniqueSymbolName = (base) => {
    let n = base || 'SYMBOL';
    for (let i = 2; usedSymbolNames.has(n); i++) n = `${base}_${i}`;
    usedSymbolNames.add(n);
    return n;
  };
  for (const it of rawItems) {
    ...
    if (!groups.has(fp)) {
      const symbolName = uniqueSymbolName(symbolVariantName(mpn, pkg, groups.size === 0));
      groups.set(fp, {
