...
export function generateBundle(input) {
  ...
  return {
    ...
    items: outItems,
    mock: !!input.mock,
    ...evaluateBundle({ items: outItems, mock: !!input.mock, confirmedFigureCount: input.confirmedFigureCount }), // 唯一晋升闸门
    warnings
  };
}
...
function safeSingle({ part, pkg, pins }, warnings) {
  ...
  const mpn = String(part.mpn || 'PART').trim() || 'PART';
  const { normalizedPackage: g, transformations, warnings: geoWarn } = normalizeGeometryDetailed(p);
