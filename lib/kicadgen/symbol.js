...
function classifyPins(pins) {
  ...
  // 顶部仅收"看起来是正电源"的名字（KLC S4.2.3）。电流路径类 power 脚（IP±/SW/LX…）
  // 按 KLC S4.2 电源变换例外处理：power_in 靠左、power_out 靠右 —— 修复 NSM2011 类
  // 电流传感器 8 个 IP 脚全被顶到上边的问题。
  const SUPPLY = /^\+?V(CC|DD|DDA|DDD|BAT|IN\b|IO|REG|REF|S\b|\+|_)|^AVDD|^DVDD|^VCC|^VDD/;
  ...
  for (const p of pins) {
    ...
    else right.push(p); // output/bidir/passive/unspecified/no_connect
  }
  // 顶部退化保护：正电源通常 ≤3 个，超出的挪到左侧避免顶排拥挤
  ...
  // 右列：信号在前、NC 沉底（SamacSys/官方库惯例，图面更干净）
  const ncs = right.filter((p) => p.type === 'no_connect');
  const sig = right.filter((p) => p.type !== 'no_connect');
  sig.sort(byNum); ncs.sort(byNum);
...
import { escSexpr, escLegacyField } from '../textsafe.js';
const esc = escSexpr;
...
/**
 * DSK-002：把 DEF 块包装成**完整的**旧版符号库。
 * 此前下载的 .lib 从 DEF 开始、到 ENDDEF 结束，缺 `EESchema-LIBRARY Version 2.4`
 * 库头与 `#End Library` 库尾，KiCad CLI 10/10 报 `Unable to convert library` ——
 * 它是符号片段，不是库文件。在线 Viewer 只解析 DRAW…ENDDRAW，不受影响。
 */
export function wrapLegacyLibrary(defBlocks, { name = '' } = {}) {
  const blocks = (Array.isArray(defBlocks) ? defBlocks : [defBlocks]).filter(Boolean);
  const out = ['EESchema-LIBRARY Version 2.4', '#encoding utf-8'];
  for (const b of blocks) {
    const defName = (/^DEF\s+(\S+)/m.exec(b) || [])[1] || name || 'SYMBOL';
    out.push('#', `# ${defName}`, '#', String(b).replace(/\s+$/, ''));
  }
  out.push('#', '#End Library', '');
  return out.join('\n');
}

/** 生成旧版 .lib DEF 块（供 kicad_part_viewer 内核渲染；下载文件需再经 wrapLegacyLibrary） */
export function generateLegacyLib({ mpn, pins }) {
  const { halfW, halfH, items, lenCells } = layout(pins);
  const W = halfW * MIL, H = halfH * MIL, LEN = lenCells * MIL;
  const out = [];
  const defName = escLegacyField(mpn);      // item 8：DEF 行是空白分隔格式，必须转义
  out.push(`DEF ${defName} U 0 40 Y Y 1 F N`);
  out.push(`F0 "U" 0 ${H + 150} 50 H V C CNN`);
  out.push(`F1 "${defName}" 0 ${-(H + 150)} 50 H V C CNN`);  // F1 值同样转义（引号/换行会破坏行结构）
  out.push(`DRAW`);
  out.push(`S ${-W} ${-H} ${W} ${H} 0 1 10 f`);
  for (const it of items) {
    const x = it.x * MIL, y = it.y * MIL;
    out.push(`X ${escLegacyField(klcName(it.pin.name).legacy)} ${escLegacyField(it.pin.number)} ${x} ${y} ${LEN} ${it.orient} 50 50 1 1 ${LTYPE[it.pin.type] || 'U'}`);
  }
  out.push(`ENDDRAW`);
  out.push(`ENDDEF`);
  return out.join('\n');
