...
function layout(pins) {
  ...
  left.forEach((p, i) => items.push({ pin: p, x: -(halfW + 2), y: yTopStart - i, orient: 'R' }));
  right.forEach((p, i) => items.push({ pin: p, x: halfW + 2, y: yTopStart - i, orient: 'L' }));
  const spread = (arr) => arr.map((_, i) => (i - (arr.length - 1) / 2) * 2).map(Math.round);
  const tx = spread(top), bx = spread(bottom);
  top.forEach((p, i) => items.push({ pin: p, x: tx[i], y: halfHcells + 2, orient: 'D' }));
  bottom.forEach((p, i) => items.push({ pin: p, x: bx[i], y: -(halfHcells + 2), orient: 'U' }));
  return { halfW, halfH: halfHcells, items };
}
...
/** 生成 KiCad 6+ .kicad_sym 文本 */
export function generateKicadSym({ mpn, footprintName, pins, description }) {
  const { halfW, halfH, items } = layout(pins);
  const W = halfW * GRID, H = halfH * GRID, LEN = 2 * GRID;
  const lines = [];
  lines.push(`(kicad_symbol_lib (version 20211014) (generator ds2kicad)`);
  lines.push(`  (symbol "${esc(mpn)}" (in_bom yes) (on_board yes)`);
  ...
  for (const it of items) {
    ...
    lines.push(`        (name "${esc(it.pin.name)}" (effects (font (size 1.27 1.27))))`);
...
export function generateLegacyLib({ mpn, pins }) {
  const { halfW, halfH, items } = layout(pins);
  const W = halfW * MIL, H = halfH * MIL, LEN = 2 * MIL;
  const out = [];
  ...
  for (const it of items) {
    ...
    out.push(`X ${it.pin.name.replace(/\s+/g, '_')} ${it.pin.number} ${x} ${y} ${LEN} ${it.orient} 50 50 1 1 ${LTYPE[it.pin.type] || 'U'}`);
  }
  out.push(`ENDDRAW`);
  out.push(`ENDDEF`);
  return out.join('\n');
