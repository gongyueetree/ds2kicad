...
import { escSexpr, safeFileName } from '../textsafe.js';
const f2 = (n) => Number(n.toFixed(3));

/** dual 右列某行对应的管脚号（1 起）；无脚返回 null。
 *  偶数：标准逆时针 n-1-i。5 脚（SOT-23-5/SC70-5）：pin4 对 pin3（底行）、pin5 对 pin1（顶行），中位空。 */
export function dualRightPinAtRow(n, half, row) {
  if (n === 5 && half === 3) {
    if (row === 0) return 5;
    if (row === half - 1) return 4;
    return null;
  }
  const idx = n - 1 - row; // 0 起下标
  return idx >= half ? idx + 1 : null;
}
...
function dualPads(pkg, pinNumbers) {
  ...
  for (let i = 0; i < half; i++) {
    ...
    const rNum = dualRightPinAtRow(n, half, i);
    if (rNum !== null) pads.push(padSmd(pinNumbers[rNum - 1], cx, y, padL, padW));
  }
...
function qfnPads(pkg, pinNumbers) {
  ...
  // 矩形 QFN/DFN：左右列由 bodyWidth（X 向）决定，上下行由 bodyLength（Y 向）决定。
  // 此前两轴都用 bodyLength，导致 3x2mm DFN 的上下行焊盘落到错误位置。
  const outerX = pkg.bodyWidth / 2 + 0.3;
  const innerX = pkg.bodyWidth / 2 - pkg.leadLength - 0.05;
  const padLX = outerX - innerX, cx = (outerX + innerX) / 2;
  const outerY = pkg.bodyLength / 2 + 0.3;
  const innerY = pkg.bodyLength / 2 - pkg.leadLength - 0.05;
  const padLY = outerY - innerY, cy = (outerY + innerY) / 2;
  const q0 = -((perSide - 1) / 2) * pitch;
  const pads = [];
  for (let i = 0; i < perSide; i++) {
    const q = q0 + i * pitch;
    pads.push(padSmd(pinNumbers[i], -cx, q, padLX, padW, { shape: i === 0 ? 'rect' : 'roundrect' })); // 左列 ↓
    pads.push(padSmd(pinNumbers[perSide + i], q, cy, padW, padLY));                                   // 底行 →
    pads.push(padSmd(pinNumbers[2 * perSide + i], cx, -q, padLX, padW));                              // 右列 ↑
    pads.push(padSmd(pinNumbers[3 * perSide + i], -q, -cy, padW, padLY));                             // 顶行 ←
  }
  ...
  return { pads, extentX: outerX, extentY: outerY, bodyX: pkg.bodyWidth / 2, bodyY: pkg.bodyLength / 2 };
}
...
function dipPads(pkg, pinNumbers) {
  ...
  // DSK-010：单一真值源。此前用 `lp?.rowSpan || pkg.rowSpan`，
  // 而封装名后缀与 3D 模型都取 pkg.rowSpan，两者不一致时会生成
  // "名字写 400 mil、焊盘却是 300 mil" 的封装。sanitizePackage 已把
  // land pattern 的孔距折算进 pkg.rowSpan，这里只认它。
  const cx = pkg.rowSpan / 2;                    // DIP 的 rowSpan 即孔心距
 ...
 */
export function generateFootprint({ mpn, pkg, pinMap, footprintNameOverride }) {
  ...
  const name = footprintNameOverride || footprintName(mpn, pkg); // item 8：名称单点来源
  ...
  out.push(`  (descr "${escSexpr(`${pkg.name} ${pkg.pinCount}-pin${pkg.drawingId ? ` (${pkg.drawingId})` : ''}, body ${pkg.bodyLength}x${pkg.bodyWidth}mm, pitch ${pkg.pitch}mm${lpNote} — generated by DS2KiCad, verify against datasheet`)}")`);
  out.push(`  (attr ${pkg.family === 'dip' ? 'through_hole' : 'smd'})`);
  out.push(`  (tags "${escSexpr(`${(pkg.type || '').trim()} ${(pkg.name || '').trim()} DS2KiCad`)}")`);
...
export function footprintName(mpn, pkg) {
  const clean = (s) => safeFileName(s, { fallback: 'PKG', max: 64 }); // item 7：文件名安全化 + KLC G1.1 字符集
