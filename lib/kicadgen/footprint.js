...
function dipPads(pkg, pinNumbers) {
  ...
  for (let i = 0; i < half; i++) {
    const y = y0 + i * pitch;
    pads.push(padTht(pinNumbers[i], -cx, y, dia, drill, i === 0));
    const rIdx = n - 1 - i;
    if (rIdx >= half) pads.push(padTht(pinNumbers[rIdx], cx, y, dia, drill, false));
  }
  return { pads, extentX: cx + dia / 2, extentY: Math.abs(y0) + dia / 2, bodyX: pkg.bodyWidth / 2, bodyY: pkg.bodyLength / 2 };
}
