...
export function generateWrl({ pkg }) {
  const parts = [];
  const bodyH = pkg.height * (pkg.family === 'dip' ? 0.85 : 0.8);
  const standoff = pkg.family === 'qfn' ? 0.02 : pkg.family === 'dip' ? 0.4 : 0.1;
  ...
  if (pkg.family === 'qfn') {
    ...
    const ll = pkg.leadLength, lt = 0.2;
    ...
    if (pkg.epLength && pkg.epWidth) {
      parts.push(box(0, f(0.05), 0, f(pkg.epWidth), f(0.1), f(pkg.epLength), MAT_LEAD));
    }
