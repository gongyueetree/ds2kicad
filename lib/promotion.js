...
export const BLOCK = {
  ...
  // v0.8.7 item 5：Package / Pin / Figure 分开的证据错误码
  PKG_EVIDENCE: 'package_field_evidence_unverified',
  PIN_EVIDENCE: 'pin_field_evidence_unverified',
  FIGURE_EVIDENCE: 'figure_evidence_missing',
...
const REASON_SCOPE = {
  ...
  'package_field_evidence_unverified': GEOMETRY_SCOPE,
  'pin_field_evidence_unverified': PIN_SCOPE,
  'figure_evidence_missing': [ASSET.FIGURES],
...
export function evaluateItem(ctx = {}) {
  ...
  const details = [];
  const perAssetKey = [];   // item 5：具体资产键级阻断（figure:<id> 等）
  ...
  const checkAnchor = (field, anchor, code = BLOCK.UNVERIFIED_EVIDENCE) => {
    // item 5：**缺失锚点 fail closed** —— 删掉 evidence 绝不能反而变可晋升
    if (!anchor) { add(code, { field, reason: 'missing_evidence_anchor' }); return; }
    const st = anchor.sourceType;
    if (st === 'reviewer') {
      if (!anchor.reviewer?.sub) add(BLOCK.UNVERIFIED_EDIT, { field, reason: 'reviewer_anchor_without_identity' });
      return;
    }
    if (st === 'unverified') add(code, { field, downgradedFrom: anchor.downgradedFrom || null });
      ...
      add(code, { field, reason: `unknown_source_type:${st}` });
    }
  };
  for (const field of relevantForEvidence) checkAnchor(field, anchors[field], BLOCK.PKG_EVIDENCE);
  ...
  if (pkg.landPattern) {
    for (const lk of ['padW', 'padL', 'rowSpan']) {
      ...
      checkAnchor(`landPattern.${lk}`, anchors[`landPattern.${lk}`], BLOCK.PKG_EVIDENCE);
    }
  }
  ...
  for (const p of ctx.pins || []) {
    for (const f of p.reviewerEdited || []) {
      ...
      if (!a || a.sourceType !== 'reviewer' || !a.reviewer?.sub) {
        add(BLOCK.PIN_EVIDENCE, { pin: p.number, field: f, reason: 'pin_edit_without_reviewer_anchor' });
      }
    }
  }
  // item 5：Figure 缺证据**只阻断该 figure:<figureId>**，不得波及 footprint/3D
  for (const f of ctx.figures || []) {
    if (!f.confirmed) continue;
    const anchor = f.evidence || f.fieldEvidence?.confirmed || f.fieldEvidence?.added;
    if (!anchor) {
      reasons.add(BLOCK.FIGURE_EVIDENCE);
      details.push({ code: BLOCK.FIGURE_EVIDENCE, figureId: f.figureId, reason: 'figure_without_evidence' });
      perAssetKey.push({ key: `figure:${f.figureId}`, code: BLOCK.FIGURE_EVIDENCE });
    }
  }
  ...
  return { nonPromotable: list.length > 0, reasons: list, details, perAssetKey, assetPromotion: split.promotable, assetBlockReasons: split.perAsset };
}

/** 汇总多个条目 + 全局上下文（mock 等）→ bundle 级判定 */
export function evaluateBundle({ items = [], mock = false, extraReasons = [], confirmedFigureCount, assetKeys = [] } = {}) {
  ...
  // item 5：具体资产键级结论 —— 某个 figure 缺证据只影响该键
  const keyBlocks = {};
  for (const it of items) for (const b of it.promotion?.perAssetKey || []) {
    (keyBlocks[b.key] ||= []).push(b.code);
  }
  const byKey = {};
  for (const key of assetKeys) {
    const kind = key.split(':')[0];
    const kindScope = kind === 'figure' ? 'figures' : kind;
    const kindBlocked = (split.perAsset[kindScope] || []).filter((r) => r !== BLOCK.FIGURE_EVIDENCE);
    const own = keyBlocks[key] || [];
    byKey[key] = { promotable: kindBlocked.length === 0 && own.length === 0, reasons: [...new Set([...kindBlocked, ...own])] };
  }
  return {
    nonPromotable: list.length > 0, reasons: list,
    assetPromotion: split.promotable, assetBlockReasons: split.perAsset,
    assetKeyPromotion: byKey
  };
}
